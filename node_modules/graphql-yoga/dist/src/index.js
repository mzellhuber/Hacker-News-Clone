...
var GraphQLServer = /** @class */ (function () {
    function GraphQLServer(props) {
        ...
        if (props.schema) {
            ...
            var directiveResolvers = props.directiveResolvers, schemaDirectives = props.schemaDirectives, resolvers = props.resolvers, typeDefs = props.typeDefs;
            var typeDefsString = buildTypeDefsString(typeDefs);
    ...
    GraphQLServer.prototype.start = function (optionsOrCallback, callback) {
        ...
        var subscriptionServerOptions = null;
        ...
        app.post(this.options.endpoint, apollo_server_express_1.graphqlExpress(function (request) { return __awaiter(_this, void 0, void 0, function () {
            var context, _a, e_1;
        ...
        if (this.options.playground) {
            var playgroundOptions = subscriptionServerOptions
                ? {
        ...
        return new Promise(function (resolve, reject) {
            var server = _this.options.https
                ? https_1.createServer(_this.options.https, app)
                : http_1.createServer(app);
            if (!subscriptionServerOptions) {
                server.listen(_this.options.port, function () {
                    callbackFunc(_this.options);
                    resolve(server);
                });
            }
            else {
                var combinedServer_1 = server;
                combinedServer_1.listen(_this.options.port, function () {
                    callbackFunc(_this.options);
                    resolve(combinedServer_1);
                });
                _this.subscriptionServer = subscriptions_transport_ws_1.SubscriptionServer.create({
                    ...
                    onConnect: subscriptionServerOptions.onConnect
                        ...
                        : function (connectionParams, webSocket) { return __awaiter(_this, void 0, void 0, function () {
...
function buildTypeDefsString(typeDefs) {
    var typeDefinitions = mergeTypeDefs(typeDefs);
    // read from .graphql file if path provided
    if (typeDefinitions.endsWith('graphql')) {
        var schemaPath = path.resolve(typeDefinitions);
        if (!fs.existsSync(schemaPath)) {
            throw new Error("No schema found for path: " + schemaPath);
        }
        typeDefinitions = graphql_import_1.importSchema(schemaPath);
    }
